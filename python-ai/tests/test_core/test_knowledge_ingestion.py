...
async def test_ingest_raw_text_chunks_embeds_and_upserts(monkeypatch):
    ...
    async def fake_embed_texts_with_model(**kwargs):
        assert kwargs["model_code"] == "embedding-qwen3-8b"
    ...
    request = KnowledgeIngestRequest(
        ...
        provider_configs=[{"provider_code": "embedding-provider"}],
        model_records=[{"model_code": "embedding-qwen3-8b"}],
    )

    response = await ingest_knowledge_document(
        request,
        provider_configs={"embedding-provider": {"provider_code": "embedding-provider"}},
        model_records={"embedding-qwen3-8b": {"model_code": "embedding-qwen3-8b"}},
    )
