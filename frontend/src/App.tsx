...
const App: React.FC = () => {
    ...
    setMessages([
      {
        id: 'welcome',
        type: 'system',
        content: '欢迎使用服务机器人。Phase 3 已启用多流程切换、恢复提示和安全防护。',
  ...
  const handleSendMessage = async (content: string, confirmSwitch = false, fixedMessageId?: string) => {
    ...
    try {
      ...
      const response = await sendMessage(activeSessionId, messageId, content, [], { confirmSwitch })
    ...
    <div className="app-shell">
      <header className="app-header">
        <div>
          ...
          <div className="text-sm text-slate-500">Phase 3 增强多流程调度</div>
        </div>
        <div className="text-xs text-slate-400">
          Session: {sessionId || 'loading'} · Socket: {socketState}
        </div>
      </header>
              ...
              <div className="panel-card prompt-card">
                ...
                <div className="prompt-actions">
                  ...
                  <button
                    ...
                    onClick={() => void handleSendMessage(pendingSwitch.content, true, pendingSwitch.messageId)}
                  >
                    暂存并切换
                  </button>
                </div>
